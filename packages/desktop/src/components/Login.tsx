      ...
      <div className={`flex-1 flex items-center justify-center ${mini ? 'p-3' : 'p-6'} overflow-hidden relative`}>
        ...
        <div className="relative z-10 w-full">
          <motion.div
            ...
            className={`${mini ? 'px-3' : `${themeClasses.card} rounded-[1.25rem] border ${themeClasses.border} overflow-hidden shadow-2xl p-6 max-w-sm mx-auto`}`}
          >
            {mini ? (
              <>
                ...
                <form onSubmit={handleLogin} className="space-y-2.5">
                  {mode === "local" ? (
                    <div className="flex gap-2">
                      <div className={`flex-1 ${themeClasses.input} border ${themeClasses.border} rounded-lg px-3 py-2.5 flex items-center overflow-hidden`}>
                      ...
                      <button
                        ...
                        className={`shrink-0 px-3.5 rounded-lg ${themeClasses.input} border ${themeClasses.border} hover:border-white/10 transition-colors font-bold text-[0.6rem] uppercase tracking-wider`}
                      >
                        Browse
                      </button>
                    </div>
                  ...
                  <div className="flex items-center gap-2">
                    <div className="relative flex-1">
                      <input
                        ...
                        className={`w-full ${themeClasses.input} border ${themeClasses.border} focus:${accentClasses.borderClass} rounded-lg px-3 py-2.5 pr-10 text-sm ${themeClasses.text} placeholder-white/20 outline-none transition-all duration-200 ring-0 focus:ring-2 ${accentClasses.focusRingClass}`}
                        autoFocus
                      />
                      <button
                        type="button"
                        onClick={() => setShowMasterPassword(!showMasterPassword)}
                        className={`absolute right-2.5 top-1/2 -translate-y-1/2 ${themeClasses.textMuted} hover:${themeClasses.text} transition-colors p-0.5`}
                      >
                        {showMasterPassword ? (
                    ...
                    <motion.button
                      ...
                      type="submit"
                      disabled={isLoading}
                      className={`shrink-0 px-5 py-2.5 rounded-lg ${accentClasses.bgClass} ${accentClasses.onContrastClass} font-bold text-[0.6rem] uppercase tracking-wider shadow ${accentClasses.shadowClass} disabled:opacity-50`}
                    >
                      {isLoading ? (
                ...
                <form onSubmit={handleLogin} className="space-y-4">
                  ...
                  {mode === "local" ? (
                    <div className="space-y-1">
                      <label className={`block text-[0.6rem] font-bold uppercase tracking-wider ${themeClasses.textMuted}`}>
                        Vault File
                      </label>
                      <div className="flex gap-2">
                        <div className={`flex-1 ${themeClasses.input} border ${themeClasses.border} rounded-xl px-4 py-3 flex items-center overflow-hidden`}>
                          <span className={`text-sm truncate ${vaultPath ? themeClasses.text : themeClasses.textMuted}`}>
                            {vaultPath ? vaultPath.split(/[\\/]/).pop() : "No vault selected"}
                          </span>
                        </div>
                  ...
                  <div className="space-y-1">
                    ...
                    <div className="relative">
                      <input
                        ...
                        placeholder={mode === "server" ? "Password" : "Master Password"}
                        className={`w-full ${themeClasses.input} border ${themeClasses.border} focus:${accentClasses.borderClass} rounded-xl px-4 py-3 pr-12 ${themeClasses.text} placeholder-white/20 outline-none transition-all duration-200 ring-0 focus:ring-4 ${accentClasses.focusRingClass}`}
                        autoFocus
                      />
                      <button
                        type="button"
                  ...
                  <motion.button
                    ...
                    type="submit"
                    disabled={isLoading}
                    className={`w-full py-3.5 rounded-xl ${accentClasses.bgClass} ${accentClasses.onContrastClass} font-bold text-[0.65rem] uppercase tracking-wider shadow-lg ${accentClasses.shadowClass} disabled:opacity-50 flex items-center justify-center gap-2`}
                  >
                    {isLoading ? (
                      <>
