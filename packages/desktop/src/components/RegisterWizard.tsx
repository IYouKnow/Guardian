...
const SERVER_STEPS = [
  { id: 1, title: "Connect to Server", description: "Enter your Guardian server address." },
  { id: 2, title: "Create Identity", description: "Setup your account credentials." },
...
export default function RegisterWizard({ mode, onRegister, onBackToLogin }: RegisterWizardProps) {
  ...
  // Theme Logic (Keeping same logic as before)
  const getThemeClasses = () => {
    switch (selectedTheme) {
      case "light":
        return { bg: "bg-[#f8fafc]", card: "bg-white/80 backdrop-blur-xl", text: "text-slate-900", textMuted: "text-slate-500", border: "border-slate-200", input: "bg-slate-100", item: "bg-slate-50", indicator: "bg-slate-200" };
      case "slate":
        return { bg: "bg-slate-950", card: "bg-slate-900/40 backdrop-blur-xl", text: "text-slate-100", textMuted: "text-slate-400", border: "border-slate-800", input: "bg-slate-800/50", item: "bg-slate-800/30", indicator: "bg-slate-800" };
      case "editor":
        return { bg: "bg-[#0d0d0d]", card: "bg-[#1a1a1a]/60 backdrop-blur-xl", text: "text-[#d4d4d4]", textMuted: "text-[#858585]", border: "border-[#333333]", input: "bg-[#252526]", item: "bg-[#1e1e1e]", indicator: "bg-[#2d2d2d" };
      case "violet":
        return { bg: "bg-[#120a1f]", card: "bg-[#23173a]/50 backdrop-blur-xl", text: "text-[#f8f8f2]", textMuted: "text-[#c9a0dc]/70", border: "border-[#4a3a6b]", input: "bg-[#2d1b4d]", item: "bg-[#1a0f2e]", indicator: "bg-[#3d2c61]" };
      default:
        return { bg: "bg-black", card: "bg-[#0a0a0a]/60 backdrop-blur-xl", text: "text-white", textMuted: "text-zinc-500", border: "border-zinc-800/50", input: "bg-zinc-900/50", item: "bg-zinc-900/30", indicator: "bg-zinc-900" };
    }
  };
  ...
  const handleSelectLocation = async () => {
    try {
      ...
      const filePath = await save({
        ...
        filters: [
          { name: "Guardian Vault", extensions: ["guardian"] },
        ],
      });
  ...
  const checkServerStatus = async () => {
    ...
    setError("");
    try {
      const res = await fetch(`${serverUrl}/auth/setup-status`);
      if (!res.ok) throw new Error("Could not connect");
      const data = await res.json();
      setServerStatus(data.status); // SETUP or READY
      return true;
    } catch (e) {
  ...
  const validateStep = async (step: number): Promise<boolean> => {
    ...
    if (mode === "local") {
      switch (step) {
        case 1:
          if (!vaultName.trim() || vaultName.trim().length < 3) { setError("Vault name must be at least 3 characters."); return false; }
          return true;
        case 2:
          if (!vaultPath) { setError("Storage location is required."); return false; }
          return true;
        case 3:
          if (masterPassword.length < 8) { setError("Master password must be at least 8 characters."); return false; }
  ...
  const handleFinish = async (e: FormEvent) => {
    ...
    try {
      if (mode === "local") {
        await onRegister({ vaultPath, masterPassword, theme: selectedTheme, accentColor: selectedAccentColor });
      } else {
        // Server Register
        await onRegister({
          url: serverUrl,
...
function ThemeOption({ id, selected, onClick, theme, accent }: any) {
  return (
    <button type="button" onClick={onClick} className={`flex items-center justify-between p-3.5 rounded-xl border transition-all ${selected ? `${accent.borderClass} ${theme.item} shadow-sm` : `${theme.border} hover:border-white/10`}`}>
...
function ColorOption({ id, selected, onClick }: any) {
  ...
  return (
    <button type="button" onClick={onClick} className={`relative aspect-square rounded-xl border-2 transition-all ${selected ? `border-white shadow-lg` : `border-transparent opacity-80`}`}>
      <div className={`w-full h-full rounded-[0.55rem] ${c.bgClass} flex items-center justify-center`}>
        {selected && <svg className="w-5 h-5 text-black" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={3} d="M5 13l4 4L19 7" /></svg>}
      </div>
    </button>
  );
}
