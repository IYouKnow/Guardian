...
import { createVault, createEmptyVault } from "@guardian/core/crypto";
...
export default function Register({ onRegister, onBackToLogin }: RegisterProps) {
  ...
  const [vaultPath, setVaultPath] = useState<string>("guardian-vault.guardian");
  ...
  const handleRegister = async (e: FormEvent) => {
    ...
    if (!vaultPath) {
      setError("Please enter a vault file name");
      return;
    }
    ...
    try {
      ...
      const base64Data = btoa(String.fromCharCode(...encryptedVault));

      // Write encrypted vault file to app's data directory
      ...
      onRegister(fileName, masterPassword);
    } catch (err) {
    ...
    <div className="flex min-h-screen bg-black text-white items-center justify-center p-6">
      <div className="w-full max-w-md">
        ...
        <form onSubmit={handleRegister} className="space-y-5">
          ...
          <div>
            <label className="block text-sm text-gray-400 mb-2">Vault File Name</label>
            <input
              type="text"
              value={vaultPath}
              onChange={(e) => {
              ...
              placeholder="guardian-vault.guardian"
              className="w-full bg-[#0a0a0a] border border-[#1a1a1a] rounded-lg px-4 py-3 text-white placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-yellow-400/50 focus:border-yellow-400/50 transition-all"
            />
            <p className="mt-1 text-xs text-gray-500">
              Vault will be saved in the app's data directory
            </p>
          </div>
